...
const App = () => {
    ...
    const [alert, setAlert] = useState({ open: false, text: 'File Uploaded Successfully!!', type: 'success' });
    ...
    const handleLoad = () => {
        fetch(`http://localhost:1234/load?file=${loadFile}`, {
            method: 'GET',
            headers: {
            ...
            .then(async (res) => {
                if (res.status === 400) {
                    ...
                    console.log(resp);
                    setAlert({ open: true, text: resp.message, type: 'error' });
                }
            })
            .catch((err) => {
                setAlert({
                    open: true,
                    text: 'Some error occurred: ' + err,
                    type: 'error',
                });
            });
        ...
        setAlert({
            open: true,
            text: 'Command issued successfully!! Loading file to the Database may take some time',
            type: 'success',
        });
    };
    ...
    useEffect(() => {
        if (status === UploadStatus.SUCCESS) {
            setAlert({ open: true, text: 'File Uploaded Successfully!!', type: 'success' });
        }
    }, [status]);
        ...
        <Paper elevation={0} style={{ padding: 0, margin: 0 }}>
            ...
            <Container style={{ padding: 16 }}>
                ...
                <Container style={{ padding: 0, margin: '16px 0' }}>
                    ...
                    <Button variant="contained" color="primary" onClick={handleLoad} style={{ marginLeft: 5 }}>
                        Load file to DB
                    </Button>
                </Container>
    ...
    async function handleUpload() {
        if (!file) {
            setAlert({ text: 'Please upload something!', type: 'warning' });
            return;
        }
